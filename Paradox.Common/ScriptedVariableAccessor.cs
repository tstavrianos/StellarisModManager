...
namespace Paradox.Common
{
    ...
    public class ScriptedVariableAccessor : IScriptedVariablesAccessor{
        ...
        public ScriptedVariableAccessor(StellarisDirectoryHelper stellarisDirectoryHelper, bool continueOnError = false, ILogger logger = null) : 
            this(stellarisDirectoryHelper, new StellarisDirectoryHelper[] { }, continueOnError, logger) {
        }

        public ScriptedVariableAccessor(StellarisDirectoryHelper stellarisDirectoryHelper,
            IEnumerable<StellarisDirectoryHelper> modDirectoryHelpers, bool continueOnError = false,ILogger logger = null) :
            this(stellarisDirectoryHelper, modDirectoryHelpers, new DirectoryWalker(), new CwParserHelper(logger), continueOnError, logger) {
        }
            ...
            ICwParserHelper cwParserHelper, bool continueOnError = false, ILogger logger = null)
        {
            ...
            foreach (var directoryHelper in StellarisDirectoryHelper.CreateCombinedList(stellarisDirectoryHelper, modDirectoryHelpers)) {
                if (Directory.Exists(directoryHelper.ScriptedVariables)) {
                    ...
                    this._logger?.Debug("{0} does not contain scripted variables", directoryHelper.ModName);
                }
        ...
        {
            var techFiles = this.DirectoryWalker.FindFilesInDirectoryTree(scriptedVariableDir, StellarisDirectoryHelper.TextMask, null, this._logger);
