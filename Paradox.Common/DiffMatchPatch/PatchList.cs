...
namespace Paradox.Common.DiffMatchPatch
{
    public static class PatchList
    {
        ...
        internal static string AddPadding(this IEnumerable<Patch> patches, short patchMargin = 4)
        {
            ...
            MatchSettings matchSettings, PatchSettings settings)
        {
            ...
            foreach (var aPatch in patches)
            {
                ...
                else
                {
                    ...
                    var actualEndLoc = endLoc == -1 ? Math.Min(startLoc + text1.Length, text.Length) : Math.Min(endLoc + Constants.MatchMaxBits, text.Length);
                                ...
                                if (aDiff.Operation != Operation.Equal)
                                {
                                    ...
                                    switch (aDiff.Operation)
                                    {
                                        case Operation.Insert:
                                            ...
                                            text = text.Insert(startLoc + index2 + offset, aDiff.Text);
                                        ...
                                        case Operation.Delete:
                                            ...
                                            var length2 = diffs.FindEquivalentLocation2(index1 + aDiff.Text.Length) - index2;
                                            text = text.Remove(startLoc + index2 + offset, length2);
                                            offset -= length2;
                                            break;
                                    }
                                }
            ...
            for (var x = 0; x < patches.Count; x++)
            {
                ...
                while (diffs.Count != 0)
                {
                    ...
                    while (diffs.Any() && patch.Length1 < patchSize - patchMargin)
                    {
                        ...
                        switch (diffType)
                        {
                            case Operation.Insert:
                                ...
                                empty = false;
                                break;
                            case Operation.Delete when patch.Diffs.Count == 1 && patch.Diffs.First().Operation == Operation.Equal && diffText.Length > 2 * patchSize:
                    ...
                    if (postcontext.Length != 0)
                    {
                        ...
                        {
                            patch.Diffs[^1] = lastDiff.Replace(lastDiff.Text + postcontext);
                        }
                        else
                        {
