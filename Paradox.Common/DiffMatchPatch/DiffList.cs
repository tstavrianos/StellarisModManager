    ...
    public static class DiffList
    {
        ...
        internal static void CleanupMerge(this List<Diff> diffs)
        {
            ...
            for (var i = 1;  i < diffs.Count - 1; i++)
            {
                ...
                var current = diffs[i];
                var next = diffs[i + 1];
                if (previous.Operation != Operation.Equal || next.Operation != Operation.Equal) continue;
        ...
        /// <returns></returns>
        internal static IEnumerable<Diff> CharsToLines(this IEnumerable<Diff> diffs, IList<string> lineArray)
        {
