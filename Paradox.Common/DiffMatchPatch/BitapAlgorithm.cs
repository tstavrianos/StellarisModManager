...
{
    internal sealed class BitapAlgorithm
    {
        ...
        private readonly int _matchDistance;

        public BitapAlgorithm(MatchSettings settings)
        {
            this._matchThreshold = settings.MatchThreshold;
            this._matchDistance = settings.MatchDistance;
        }
        ...
        public int Match(string text, string pattern, int loc)
        {
            ...
            double scoreThreshold = this._matchThreshold;
            // Is there a nearby exact match? (speedup)
            var bestLoc = text.IndexOf(pattern, loc, StringComparison.Ordinal);
            if (bestLoc != -1)
            {
                scoreThreshold = Math.Min(this.MatchBitapScore(0, bestLoc, loc,
            ...
            for (var d = 0; d < pattern.Length; d++)
            {
                ...
                for (var j = finish; j >= start; j--)
                {
                    ...
                    var score = this.MatchBitapScore(d, j - 1, loc, pattern);
        ...
        private double MatchBitapScore(int e, int x, int loc, string pattern)
        {
            ...
            if (this._matchDistance == 0)
            {
                // Dodge divide by zero error.
                return proximity == 0 ? accuracy : 1.0;
            }
            return accuracy + proximity / (float) this._matchDistance;
        }
    }
}