...
using Splat;
using Steam.Common.WebAPI.Responces;
...
namespace Steam.Common.WebAPI.Requests
{
    public sealed class SteamRemoteStorage: ISteamRemoteStorage, IEnableLogger
    {
        ...
        private List<CachedResponce<PublishedFileDetail>> _publishedFileDetailsCached;
        ...
        private void CheckCache<T>(ref List<CachedResponce<T>> property, FileSystemInfo file)
        {
            if (!(property is null)) return;
            if (file.Exists) {
                try {
                    property = JsonConvert.DeserializeObject<List<CachedResponce<T>>>(File.ReadAllText(file.FullName));
                } catch (Exception ex) {
                    this.Log().Error(ex, "[Steam] Unable to load cache");
                    property = new List<CachedResponce<T>>();
                }
            } else {
                property = new List<CachedResponce<T>>();
            }
        ...
        public Task<GetPublishedFileDetailsResponse> GetPublishedFileDetailsAsync(IReadOnlyList<ulong> fileIds)
        {
        ...
        public async Task<GetPublishedFileDetailsResponse> GetPublishedFileDetailsAsync(IReadOnlyList<string> fileIds)
        {
            ...
            if (this._cached)
            {
                    ...
                    (!this._publishedFileDetailsCacheFile.LastWriteTime.ExpiredSince(10)))
                {
                    foreach (var item in fileIds.Select(fileId => this._publishedFileDetailsCached.FirstOrDefault(x =>
                        $"{x.Responce.PublishedFileId}" == fileId)).Where(item => item != null))
                    {
                        parsedResponse.Response.PublishedFileDetails.Add(item.Responce);
                    }
            ...
            {
                this._publishedFileDetailsCached.RemoveAll(x => x.Responce.PublishedFileId == item.PublishedFileId);
                this._publishedFileDetailsCached.Add(CachedResponce<PublishedFileDetail>.FromResponce(item));
            }
